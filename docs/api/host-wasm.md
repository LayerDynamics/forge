# host:wasm API Reference

The `host:wasm` module provides WebAssembly runtime capabilities with WASI support and capability-based security.
...
```typescript
import { compileFile } from "host:wasm";
...
```typescript
import { dropModule } from "host:wasm";
...
```typescript
import { call, getExports, memory } from "host:wasm";
