...
fn cmd_dev(app_dir: &Path) -> Result<()> {
    ...
    if forge_host.to_string_lossy() == "__cargo_run__" {
        ...
        if !status.success() {
            bail!("forge-runtime failed");
        }
        return Ok(());
    }

    // Run the found forge-runtime binary
    let status = Command::new(&forge_host)
        .args(["--app-dir", &app_dir.display().to_string(), "--dev"])
        .status()
        .context("Failed to run forge-runtime")?;

    if !status.success() {
        bail!("forge-runtime exited with error");
    }
...
fn bundle_with_esbuild(app_dir: &Path, dist_dir: &Path, framework: &Framework) -> Result<()> {
    // For Svelte/Vue, use the dist/web dir (where transformed files are)
    // For other frameworks, use the original web dir
    let entry_dir = match framework {
        Framework::Svelte | Framework::Vue => dist_dir.join("web"),
        _ => app_dir.join("web"),
    };
    let entry = match find_entry_point(&entry_dir) {
        Some(e) => e,
        None => {
    ...
    // For Svelte, we need to configure proper npm resolution
    let svelte_config = if *framework == Framework::Svelte {
        r#"
  alias: {
    "svelte": "npm:svelte@4",
    "svelte/internal": "npm:svelte@4/internal",
    "svelte/internal/disclose-version": "npm:svelte@4/internal/disclose-version",
  },
  nodePaths: [],
"#
    } else {
...
fn transform_svelte_files(app_dir: &Path, dist_dir: &Path) -> Result<()> {
    ...
    // Create Svelte transform script using svelte/compiler with TypeScript preprocessing
    let transform_script = format!(
        r#"
import {{ compile, preprocess }} from "npm:svelte@4/compiler";
import {{ walk }} from "https://deno.land/std@0.208.0/fs/walk.ts";
...
for await (const entry of walk(webDir, {{ exts: [".svelte"] }})) {{
  ...
  try {{
    ...
    const preprocessed = await preprocess(content, tsPreprocessor, {{
      filename: entry.name,
    }});
...
// Update imports in main.ts/main.js to reference .js instead of .svelte
for await (const entry of walk(distDir, {{ exts: [".ts", ".js"] }})) {{
  const content = await Deno.readTextFile(entry.path);
  if (content.includes(".svelte")) {{
    const updated = content.replace(/\.svelte(['"])/g, ".js$1");
